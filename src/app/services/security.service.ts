...
export class SecurityService {
  private clientSettings = {
    ...
    clientId:
      '131910832628-2hnl92lqenqpom4jo0r5hh7bcqovc4af.apps.googleusercontent.com',
  ...
  public signIn(): Promise<gapi.auth2.GoogleUser> {
    if (this.inited)
      return gapi.auth2
        .getAuthInstance()
        .signIn({ ux_mode: 'redirect', redirect_uri: window.location.href });

    throw "can't signIn SecurityService not inited";
  }

  public signOut(): any {
    if (this.inited) return gapi.auth2.getAuthInstance().signOut();
