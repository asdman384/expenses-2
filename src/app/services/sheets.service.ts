...
export class SheetsService {
    ...
    private onInfoLoaded = new Promise((resolve, reject) => this.infoLoadResolve = resolve);
    ...
    public init(): void {
        ...
        this.loadSpreadSheetInfo()
            .then(this.initSpreadSheet);
    }
    ...
    private initSpreadSheet = (spreadSheet: gapi.client.sheets.Spreadsheet) => {
        ...
        spreadSheet.sheets.forEach(sheet => {
            hasCategoriesSheet = hasCategoriesSheet || sheet.properties.title === this.categoriesSheetTitle;
            hasCurrentYearSheet = hasCurrentYearSheet || sheet.properties.title === this.currentYearSheetTitle;
        });

        if (!hasCategoriesSheet) {
            //add Sheet `categories`
            this.addSheet(this.categoriesSheetTitle)
                .then(x => console.log(`Sheet ${this.categoriesSheetTitle} added`, x));
        }

        if (!hasCurrentYearSheet) {
            //add Sheet `currentYear`
            this.addSheet(this.currentYearSheetTitle)
                .then(x => console.log(`Sheet ${this.currentYearSheetTitle} added`, x));
        }
    };

    private addSheet(title: string): Promise<gapi.client.Response<gapi.client.sheets.BatchUpdateSpreadsheetResponse>> {
        ...
        return this.onInfoLoaded
            ...
            .then(response => {
                ...
                if (response.result.values)
                    categories = response.result.values.map(value => ({ id: value[0], name: value[1] }));
